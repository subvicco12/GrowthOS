...
final class GrowthOS_REST {
 ...
 public static function metric_goals(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$t=$wpdb->prefix.'growthos_metric_goals';if($request->get_method()==='GET'){$site=(int)$request->get_param('site_id');$where=$site?$wpdb->prepare("WHERE site_id=%d",$site):'';$rows=$wpdb->get_results("SELECT * FROM $t $where ORDER BY metric_key",ARRAY_A);return new WP_REST_Response(['items'=>$rows],200);}$site=(int)$request->get_param('site_id');$metric=sanitize_key((string)$request->get_param('metric_key'));$direction=sanitize_key((string)$request->get_param('direction'));if(!$site||!$metric||!in_array($direction,['increase','decrease'],true))return new WP_REST_Response(['error'=>'INVALID_METRIC_GOAL'],400);$target=$request->get_param('target_value');$data=['site_id'=>$site,'metric_key'=>$metric,'direction'=>$direction,'unit'=>sanitize_text_field((string)$request->get_param('unit')),'target_value'=>is_numeric($target)?(float)$target:null,'updated_at'=>current_time('mysql')];$wpdb->replace($t,$data);return new WP_REST_Response(['ok'=>true,'site_id'=>$site,'metric_key'=>$metric,'direction'=>$direction],200);
 }
