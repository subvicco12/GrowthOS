...
final class GrowthOS_REST {
 public static function register_routes(): void {
  ...
  register_rest_route('growthos/v1','/roi',['methods'=>'GET','callback'=>[self::class,'roi'],'permission_callback'=>fn()=>current_user_can('growthos_access')]);
  register_rest_route('growthos/v1','/revenue-events',['methods'=>['GET','POST'],'callback'=>[self::class,'revenue_events'],'permission_callback'=>fn()=>current_user_can('growthos_access')&&($_SERVER['REQUEST_METHOD']==='GET'||current_user_can('growthos_manage'))]);
  register_rest_route('growthos/v1','/commercial-priorities',['methods'=>'GET','callback'=>[self::class,'commercial_priorities'],'permission_callback'=>fn()=>current_user_can('growthos_access')]);
  register_rest_route('growthos/v1','/commercial-quality',['methods'=>'GET','callback'=>[self::class,'commercial_quality'],'permission_callback'=>fn()=>current_user_can('growthos_access')]);
  register_rest_route('growthos/v1','/revenue-attribution',['methods'=>'GET','callback'=>[self::class,'revenue_attribution'],'permission_callback'=>fn()=>current_user_can('growthos_access')]);
 ...
 public static function cost_events(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$t=$wpdb->prefix.'growthos_cost_events';if($request->get_method()==='GET'){$site=(int)$request->get_param('site_id');$where=$site?$wpdb->prepare("WHERE site_id=%d",$site):'';$rows=$wpdb->get_results("SELECT * FROM $t $where ORDER BY occurred_at DESC,id DESC LIMIT 200",ARRAY_A);return new WP_REST_Response(['items'=>$rows],200);}$site=(int)$request->get_param('site_id');$type=sanitize_key((string)$request->get_param('cost_type'));$currency=strtoupper(sanitize_text_field((string)$request->get_param('currency')));$amount=$request->get_param('amount');$evidence=sanitize_textarea_field((string)$request->get_param('evidence'));if(!$site||!$type||!is_numeric($amount)||(float)$amount<0||!preg_match('/^[A-Z]{3}$/',$currency)||$evidence==='')return new WP_REST_Response(['error'=>'INVALID_COST_EVENT'],400);$eid=(int)$request->get_param('execution_id');if($eid){$ok=$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}growthos_executions WHERE id=%d AND site_id=%d",$eid,$site));if(!$ok)return new WP_REST_Response(['error'=>'EXECUTION_NOT_FOUND'],404);}$occurred=sanitize_text_field((string)$request->get_param('occurred_at'));if(!$occurred)$occurred=current_time('mysql');$wpdb->insert($t,['site_id'=>$site,'execution_id'=>$eid?:null,'cost_type'=>$type,'amount'=>(float)$amount,'currency'=>$currency,'evidence'=>$evidence,'occurred_at'=>$occurred]);return new WP_REST_Response(['id'=>(int)$wpdb->insert_id],201);
 }
 public static function portfolio_commercial(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$p=$wpdb->prefix.'growthos_';$rows=$wpdb->get_results("SELECT s.id site_id,s.name,s.domain,v.currency,COALESCE(v.revenue,0) revenue,COALESCE(c.cost,0) cost FROM {$p}sites s JOIN (SELECT site_id,currency,SUM(amount) revenue FROM {$p}revenue_events GROUP BY site_id,currency) v ON v.site_id=s.id LEFT JOIN (SELECT site_id,currency,SUM(amount) cost FROM {$p}cost_events GROUP BY site_id,currency) c ON c.site_id=s.id AND c.currency=v.currency ORDER BY v.currency,v.revenue DESC",ARRAY_A);foreach($rows as &$x){$rev=(float)$x['revenue'];$cost=(float)$x['cost'];$x['revenue']=$rev;$x['cost']=$cost;$x['net_value']=$rev-$cost;$x['roi_percent']=$cost>0?(($rev-$cost)/$cost)*100:null;}unset($x);return new WP_REST_Response(['items'=>$rows,'count'=>count($rows)],200);
 }
 public static function roi(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$p=$wpdb->prefix.'growthos_';$site=(int)$request->get_param('site_id');$where=$site?$wpdb->prepare("AND e.site_id=%d",$site):'';$rows=$wpdb->get_results("SELECT e.id execution_id,e.site_id,r.title,s.name site_name,x.currency,x.revenue,COALESCE(c.cost,0) cost FROM {$p}executions e JOIN {$p}recommendations r ON r.id=e.recommendation_id JOIN {$p}sites s ON s.id=e.site_id JOIN (SELECT execution_id,currency,SUM(amount) revenue FROM {$p}revenue_events WHERE execution_id IS NOT NULL GROUP BY execution_id,currency) x ON x.execution_id=e.id LEFT JOIN (SELECT execution_id,currency,SUM(amount) cost FROM {$p}cost_events WHERE execution_id IS NOT NULL GROUP BY execution_id,currency) c ON c.execution_id=e.id AND c.currency=x.currency WHERE e.verification_status='passed' $where ORDER BY e.id DESC",ARRAY_A);foreach($rows as &$x){$rev=(float)$x['revenue'];$cost=(float)$x['cost'];$x['revenue']=$rev;$x['cost']=$cost;$x['net_value']=$rev-$cost;$x['roi_percent']=$cost>0?(($rev-$cost)/$cost)*100:null;}unset($x);return new WP_REST_Response(['items'=>$rows,'count'=>count($rows)],200);
 }
 public static function revenue_events(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$t=$wpdb->prefix.'growthos_revenue_events';if($request->get_method()==='GET'){$site=(int)$request->get_param('site_id');$where=$site?$wpdb->prepare("WHERE site_id=%d",$site):'';$rows=$wpdb->get_results("SELECT * FROM $t $where ORDER BY occurred_at DESC,id DESC LIMIT 200",ARRAY_A);return new WP_REST_Response(['items'=>$rows],200);}$site=(int)$request->get_param('site_id');$type=sanitize_key((string)$request->get_param('event_type'));$currency=strtoupper(sanitize_text_field((string)$request->get_param('currency')));$amount=$request->get_param('amount');$evidence=sanitize_textarea_field((string)$request->get_param('evidence'));if(!$site||!$type||!is_numeric($amount)||!preg_match('/^[A-Z]{3}$/',$currency)||$evidence==='')return new WP_REST_Response(['error'=>'INVALID_REVENUE_EVENT'],400);$eid=(int)$request->get_param('execution_id');if($eid){$ok=$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}growthos_executions WHERE id=%d AND site_id=%d AND verification_status='passed'",$eid,$site));if(!$ok)return new WP_REST_Response(['error'=>'VERIFIED_EXECUTION_REQUIRED'],409);}$occurred=sanitize_text_field((string)$request->get_param('occurred_at'));if(!$occurred)$occurred=current_time('mysql');$wpdb->insert($t,['site_id'=>$site,'execution_id'=>$eid?:null,'event_type'=>$type,'amount'=>(float)$amount,'currency'=>$currency,'source'=>sanitize_key((string)$request->get_param('source')),'evidence'=>$evidence,'occurred_at'=>$occurred]);return new WP_REST_Response(['id'=>(int)$wpdb->insert_id],201);
 }
 public static function commercial_priorities(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$p=$wpdb->prefix.'growthos_';$site=(int)$request->get_param('site_id');$where=$site?$wpdb->prepare("AND e.site_id=%d",$site):'';$rows=$wpdb->get_results("SELECT e.id execution_id,e.site_id,r.title,r.category,s.name site_name,v.currency,v.revenue,COALESCE(c.cost,0) cost,v.events revenue_events,v.evidence_events revenue_evidence,COALESCE(c.events,0) cost_events,COALESCE(c.evidence_events,0) cost_evidence FROM {$p}executions e JOIN {$p}recommendations r ON r.id=e.recommendation_id JOIN {$p}sites s ON s.id=e.site_id JOIN (SELECT execution_id,currency,SUM(amount) revenue,COUNT(*) events,SUM(CASE WHEN evidence<>'' THEN 1 ELSE 0 END) evidence_events FROM {$p}revenue_events WHERE execution_id IS NOT NULL GROUP BY execution_id,currency) v ON v.execution_id=e.id LEFT JOIN (SELECT execution_id,currency,SUM(amount) cost,COUNT(*) events,SUM(CASE WHEN evidence<>'' THEN 1 ELSE 0 END) evidence_events FROM {$p}cost_events WHERE execution_id IS NOT NULL GROUP BY execution_id,currency) c ON c.execution_id=e.id AND c.currency=v.currency WHERE e.verification_status='passed' $where",ARRAY_A);$out=[];foreach($rows as $x){$supported=((int)$x['revenue_events']>0&&(int)$x['revenue_evidence']===(int)$x['revenue_events']&&((int)$x['cost_events']===0||(int)$x['cost_evidence']===(int)$x['cost_events']));if(!$supported)continue;$rev=(float)$x['revenue'];$cost=(float)$x['cost'];$x['revenue']=$rev;$x['cost']=$cost;$x['net_value']=$rev-$cost;$x['roi_percent']=$cost>0?(($rev-$cost)/$cost)*100:null;$x['evidence_quality']='supported';$out[]=$x;}usort($out,fn($a,$b)=>$b['net_value']<=>$a['net_value']);return new WP_REST_Response(['items'=>$out,'count'=>count($out),'rule'=>'verified_execution_and_complete_recorded_evidence'],200);
 }
 public static function commercial_quality(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$p=$wpdb->prefix.'growthos_';$site=(int)$request->get_param('site_id');$where=$site?$wpdb->prepare("WHERE e.site_id=%d",$site):'';$rows=$wpdb->get_results("SELECT e.id execution_id,e.site_id,r.title,s.name site_name,COUNT(DISTINCT v.id) revenue_events,COUNT(DISTINCT c.id) cost_events,COUNT(DISTINCT CASE WHEN v.evidence<>'' THEN v.id END) revenue_evidence,COUNT(DISTINCT CASE WHEN c.evidence<>'' THEN c.id END) cost_evidence FROM {$p}executions e JOIN {$p}recommendations r ON r.id=e.recommendation_id JOIN {$p}sites s ON s.id=e.site_id LEFT JOIN {$p}revenue_events v ON v.execution_id=e.id LEFT JOIN {$p}cost_events c ON c.execution_id=e.id WHERE e.verification_status='passed' $where GROUP BY e.id,e.site_id,r.title,s.name ORDER BY e.id DESC",ARRAY_A);foreach($rows as &$x){$x['revenue_events']=(int)$x['revenue_events'];$x['cost_events']=(int)$x['cost_events'];$x['revenue_evidence']=(int)$x['revenue_evidence'];$x['cost_evidence']=(int)$x['cost_evidence'];$x['quality']=$x['revenue_events']===0?'no_revenue_data':(($x['revenue_evidence']<$x['revenue_events']||($x['cost_events']>0&&$x['cost_evidence']<$x['cost_events']))?'incomplete':'supported');}unset($x);return new WP_REST_Response(['items'=>$rows,'count'=>count($rows)],200);
 }
 public static function revenue_attribution(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$p=$wpdb->prefix.'growthos_';$site=(int)$request->get_param('site_id');$where=$site?$wpdb->prepare("AND e.site_id=%d",$site):'';$rows=$wpdb->get_results("SELECT e.id execution_id,e.site_id,r.title,r.category,s.name site_name,v.currency,COUNT(v.id) events,SUM(v.amount) attributed_amount,MIN(v.occurred_at) first_event,MAX(v.occurred_at) last_event FROM {$p}executions e JOIN {$p}recommendations r ON r.id=e.recommendation_id JOIN {$p}sites s ON s.id=e.site_id JOIN {$p}revenue_events v ON v.execution_id=e.id WHERE e.verification_status='passed' $where GROUP BY e.id,e.site_id,r.title,r.category,s.name,v.currency ORDER BY attributed_amount DESC",ARRAY_A);return new WP_REST_Response(['items'=>$rows,'count'=>count($rows)],200);
 }
 public static function revenue_summary(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$t=$wpdb->prefix.'growthos_revenue_events';$site=(int)$request->get_param('site_id');$where=$site?$wpdb->prepare("WHERE site_id=%d",$site):'';$rows=$wpdb->get_results("SELECT currency,event_type,COUNT(*) events,SUM(amount) amount,SUM(CASE WHEN execution_id IS NOT NULL THEN amount ELSE 0 END) attributed_amount FROM $t $where GROUP BY currency,event_type ORDER BY currency,event_type",ARRAY_A);return new WP_REST_Response(['site_id'=>$site?:null,'groups'=>$rows],200);
 }
 public static function metric_goals(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$t=$wpdb->prefix.'growthos_metric_goals';if($request->get_method()==='GET'){$site=(int)$request->get_param('site_id');$where=$site?$wpdb->prepare("WHERE site_id=%d",$site):'';$rows=$wpdb->get_results("SELECT * FROM $t $where ORDER BY metric_key",ARRAY_A);return new WP_REST_Response(['items'=>$rows],200);}
  $site=(int)$request->get_param('site_id');$metric=sanitize_key((string)$request->get_param('metric_key'));$direction=sanitize_key((string)$request->get_param('direction'));if(!$site||!$metric||!in_array($direction,['increase','decrease'],true))return new WP_REST_Response(['error'=>'INVALID_METRIC_GOAL'],400);
  $sites=$wpdb->prefix.'growthos_sites';if(!(int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $sites WHERE id=%d",$site)))return new WP_REST_Response(['error'=>'SITE_NOT_FOUND'],404);
  $target=$request->get_param('target_value');$unit=sanitize_text_field((string)$request->get_param('unit'));$now=current_time('mysql');$before=$wpdb->get_row($wpdb->prepare("SELECT * FROM $t WHERE site_id=%d AND metric_key=%s",$site,$metric),ARRAY_A);
  $sql="INSERT INTO $t (site_id,metric_key,direction,unit,target_value,created_at,updated_at) VALUES (%d,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE direction=VALUES(direction),unit=VALUES(unit),target_value=VALUES(target_value),updated_at=VALUES(updated_at)";
  $targetSql=is_numeric($target)?(string)(float)$target:null;$ok=$wpdb->query($wpdb->prepare($sql,$site,$metric,$direction,$unit,$targetSql,$now,$now));if($ok===false)return new WP_REST_Response(['error'=>'METRIC_GOAL_SAVE_FAILED'],500);
  $after=$wpdb->get_row($wpdb->prepare("SELECT * FROM $t WHERE site_id=%d AND metric_key=%s",$site,$metric),ARRAY_A);$audit=$wpdb->prefix.'growthos_audit_events';$wpdb->insert($audit,['actor_id'=>get_current_user_id(),'site_id'=>$site,'action'=>$before?'metric_goal.updated':'metric_goal.created','object_type'=>'metric_goal','object_id'=>(string)($after['id']??$metric),'before_data'=>wp_json_encode($before),'after_data'=>wp_json_encode($after),'created_at'=>$now]);
  return new WP_REST_Response(['ok'=>true,'goal'=>$after],200);
 }
 public static function impact(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$p=$wpdb->prefix.'growthos_';$site=(int)$request->get_param('site_id');$where=$site?$wpdb->prepare("AND e.site_id=%d",$site):'';
  $rows=$wpdb->get_results("SELECT e.id execution_id,e.site_id,r.title,r.category,s.name site_name,m.metric_key,m.unit,m.baseline_value baseline,m.observed_value after_value,g.direction goal_direction,g.target_value FROM {$p}executions e JOIN {$p}recommendations r ON r.id=e.recommendation_id JOIN {$p}sites s ON s.id=e.site_id JOIN {$p}measurements m ON m.execution_id=e.id LEFT JOIN {$p}metric_goals g ON g.site_id=e.site_id AND g.metric_key=m.metric_key WHERE e.verification_status='passed' $where ORDER BY m.created_at DESC,m.id DESC",ARRAY_A);
  $out=[];foreach($rows as $x){if($x['baseline']===null||$x['after_value']===null)continue;$b=(float)$x['baseline'];$a=(float)$x['after_value'];$delta=$a-$b;$pct=$b!=0?($delta/abs($b))*100:null;$x['baseline']=$b;$x['after_value']=$a;$x['delta']=$delta;$x['change_percent']=$pct;$x['direction']=$delta>0?'up':($delta<0?'down':'flat');$goal=$x['goal_direction']??null;$target=$x['target_value']!==null?(float)$x['target_value']:null;$x['target_value']=$target;$x['outcome']=!$goal?'unclassified':($delta==0?'neutral':(($goal==='increase'&&$delta>0)||($goal==='decrease'&&$delta<0)?'improved':'regressed'));$x['target_met']=$target===null?null:($goal==='increase'?$a>=$target:$a<=$target);$out[]=$x;}return new WP_REST_Response(['items'=>$out,'count'=>count($out)],200);
 }
 public static function measurements(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$p=$wpdb->prefix.'growthos_';if($request->get_method()==='GET'){$site=(int)$request->get_param('site_id');$eid=(int)$request->get_param('execution_id');$clauses=[];$args=[];if($site){$clauses[]='m.site_id=%d';$args[]=$site;}if($eid){$clauses[]='m.execution_id=%d';$args[]=$eid;}$where=$clauses?'WHERE '.implode(' AND ',$clauses):'';$sql="SELECT m.*,e.recommendation_id,r.title,s.name site_name FROM {$p}measurements m JOIN {$p}executions e ON e.id=m.execution_id JOIN {$p}recommendations r ON r.id=e.recommendation_id JOIN {$p}sites s ON s.id=m.site_id $where ORDER BY m.created_at DESC,m.id DESC LIMIT 200";if($args)$sql=$wpdb->prepare($sql,...$args);$rows=$wpdb->get_results($sql,ARRAY_A);return new WP_REST_Response(['items'=>$rows,'count'=>count($rows)],200);}if(!current_user_can('growthos_manage'))return new WP_REST_Response(['error'=>'FORBIDDEN'],403);$eid=(int)$request->get_param('execution_id');$e=$wpdb->get_row($wpdb->prepare("SELECT id,site_id,verification_status FROM {$p}executions WHERE id=%d",$eid),ARRAY_A);if(!$e)return new WP_REST_Response(['error'=>'EXECUTION_NOT_FOUND'],404);if($e['verification_status']!=='passed')return new WP_REST_Response(['error'=>'VERIFIED_EXECUTION_REQUIRED'],409);$key=sanitize_key((string)$request->get_param('metric_key'));$evidence=sanitize_textarea_field((string)$request->get_param('evidence'));if(!$key||!$evidence)return new WP_REST_Response(['error'=>'METRIC_AND_EVIDENCE_REQUIRED'],400);$base=$request->get_param('baseline_value');$obs=$request->get_param('observed_value');$data=['execution_id'=>$eid,'site_id'=>(int)$e['site_id'],'metric_key'=>$key,'baseline_value'=>is_numeric($base)?$base:null,'observed_value'=>is_numeric($obs)?$obs:null,'unit'=>sanitize_text_field((string)$request->get_param('unit')),'window_start'=>sanitize_text_field((string)$request->get_param('window_start'))?:null,'window_end'=>sanitize_text_field((string)$request->get_param('window_end'))?:null,'evidence'=>$evidence];$wpdb->insert($p.'measurements',$data);return new WP_REST_Response(['id'=>(int)$wpdb->insert_id]+$data,201);
 }
 ...
 public static function system_health(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$j=$wpdb->prefix.'growthos_jobs';$c=$wpdb->prefix.'growthos_connectors';$now=current_time('mysql');$queued=(int)$wpdb->get_var("SELECT COUNT(*) FROM $j WHERE status='queued'");$failed=(int)$wpdb->get_var("SELECT COUNT(*) FROM $j WHERE status='failed'");$blocked=(int)$wpdb->get_var("SELECT COUNT(*) FROM $j WHERE status='blocked'");$running=(int)$wpdb->get_var("SELECT COUNT(*) FROM $j WHERE status='running'");$degraded=(int)$wpdb->get_var("SELECT COUNT(*) FROM $c WHERE status='degraded'");$next=wp_next_scheduled(GrowthOS_Jobs::HOOK);$status=($failed>0||$blocked>0||$degraded>0)?'attention':'healthy';return new WP_REST_Response(['status'=>$status,'jobs'=>['queued'=>$queued,'running'=>$running,'failed'=>$failed,'blocked'=>$blocked],'degraded_connectors'=>$degraded,'worker'=>['scheduled'=>(bool)$next,'next_run'=>$next?gmdate('c',$next):null],'checked_at'=>$now],200);
 }
 ...
 public static function package_gap(WP_REST_Request $request): WP_REST_Response {
  ...
  $seen=[];$evidence=[];foreach($rows as &$x){$features=json_decode($x['features']?:'[]',true);$x['features']=is_array($features)?$features:[];foreach($x['features'] as $v){$label=is_array($v)?($v['name']??''):$v;$key=sanitize_key((string)$label);if(!$key)continue;$cid=(int)$x['competitor_id'];$seen[$key][$cid]=true;$evidence[$key][$cid]=[$x['competitor'],$x['evidence_url']];}}
  unset($x);$freq=[];foreach($seen as $key=>$ids)$freq[$key]=count($ids);arsort($freq);$gaps=[];foreach($freq as $key=>$count)if(!isset($ownKeys[$key]))$gaps[]=['feature_key'=>$key,'competitor_mentions'=>$count,'distinct_competitors'=>$count,'evidence'=>array_values($evidence[$key])];
 ...
 public static function save_competitor(WP_REST_Request $request): WP_REST_Response {global $wpdb;$t=$wpdb->prefix.'growthos_competitors';$site=(int)$request->get_param('site_id');$name=sanitize_text_field($request->get_param('name'));$domain=strtolower(preg_replace('#^https?://#','',trim((string)$request->get_param('domain'))));$domain=rtrim($domain,'/');$e=esc_url_raw($request->get_param('evidence_url'));if(!$site||!$name||!$domain||!$e)return new WP_REST_Response(['ok'=>false,'code'=>'EVIDENCE_REQUIRED'],400);$now=current_time('mysql');$sql=$wpdb->prepare("INSERT INTO $t (site_id,name,domain,evidence_url,status,verified_at,created_at) VALUES (%d,%s,%s,%s,'active',%s,%s) ON DUPLICATE KEY UPDATE name=VALUES(name),evidence_url=VALUES(evidence_url),status='active',verified_at=VALUES(verified_at)",$site,$name,$domain,$e,$now,$now);$ok=$wpdb->query($sql);if($ok===false)return new WP_REST_Response(['ok'=>false,'code'=>'COMPETITOR_SAVE_FAILED'],500);$id=(int)$wpdb->get_var($wpdb->prepare("SELECT id FROM $t WHERE site_id=%d AND domain=%s",$site,$domain));return new WP_REST_Response(['ok'=>true,'id'=>$id],$wpdb->insert_id?201:200);}
 ...
 public static function save_competitor_package(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$t=$wpdb->prefix.'growthos_competitor_packages';$ct=$wpdb->prefix.'growthos_competitors';$cid=(int)$request->get_param('competitor_id');$name=sanitize_text_field((string)$request->get_param('plan_name'));$key=sanitize_key((string)$request->get_param('plan_key'));$e=esc_url_raw((string)$request->get_param('evidence_url'));if(!$cid||!$name||!$key||!$e)return new WP_REST_Response(['ok'=>false,'code'=>'EVIDENCE_REQUIRED'],400);if(!(int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $ct WHERE id=%d",$cid)))return new WP_REST_Response(['ok'=>false,'code'=>'COMPETITOR_NOT_FOUND'],404);
  $price=$request->get_param('price_amount');$data=['plan_name'=>$name,'price_amount'=>$price!==null&&$price!==''?(float)$price:null,'currency'=>sanitize_text_field((string)$request->get_param('currency')),'billing_period'=>sanitize_text_field((string)$request->get_param('billing_period')),'features'=>wp_json_encode((array)$request->get_param('features')),'evidence_url'=>$e,'observed_at'=>current_time('mysql')];$existing=(int)$wpdb->get_var($wpdb->prepare("SELECT id FROM $t WHERE competitor_id=%d AND plan_key=%s",$cid,$key));
  if($existing){$ok=$wpdb->update($t,$data,['id'=>$existing]);$id=$existing;$status=200;}else{$ok=$wpdb->insert($t,['competitor_id'=>$cid,'plan_key'=>$key]+$data);$id=(int)$wpdb->insert_id;$status=201;}if($ok===false)return new WP_REST_Response(['ok'=>false,'code'=>'COMPETITOR_PACKAGE_SAVE_FAILED'],500);return new WP_REST_Response(['ok'=>true,'id'=>$id,'updated'=>(bool)$existing],$status);
 }
