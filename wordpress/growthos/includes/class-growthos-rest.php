...
final class GrowthOS_REST {
 ...
 public static function impact(WP_REST_Request $request): WP_REST_Response {
  global $wpdb;$p=$wpdb->prefix.'growthos_';$site=(int)$request->get_param('site_id');$where=$site?$wpdb->prepare("AND e.site_id=%d",$site):'';$rows=$wpdb->get_results("SELECT e.id execution_id,e.site_id,r.title,r.category,s.name site_name,m.metric_key,m.unit,MAX(CASE WHEN m.period='baseline' THEN m.value END) baseline,MAX(CASE WHEN m.period='after' THEN m.value END) after_value FROM {$p}executions e JOIN {$p}recommendations r ON r.id=e.recommendation_id JOIN {$p}sites s ON s.id=e.site_id JOIN {$p}measurements m ON m.execution_id=e.id WHERE e.verification_status='passed' $where GROUP BY e.id,e.site_id,r.title,r.category,s.name,m.metric_key,m.unit ORDER BY e.id DESC",ARRAY_A);$out=[];foreach($rows as $x){if($x['baseline']===null||$x['after_value']===null)continue;$b=(float)$x['baseline'];$a=(float)$x['after_value'];$delta=$a-$b;$pct=$b!=0?($delta/abs($b))*100:null;$x['baseline']=$b;$x['after_value']=$a;$x['delta']=$delta;$x['change_percent']=$pct;$x['direction']=$delta>0?'up':($delta<0?'down':'flat');$goal=$wpdb->get_row($wpdb->prepare("SELECT direction,target_value FROM {$p}metric_goals WHERE site_id=%d AND metric_key=%s",(int)$x['site_id'],$x['metric_key']),ARRAY_A);$x['goal_direction']=$goal['direction']??null;$x['target_value']=isset($goal['target_value'])?(float)$goal['target_value']:null;$x['outcome']=!$goal?'unclassified':($delta==0?'neutral':(($goal['direction']==='increase'&&$delta>0)||($goal['direction']==='decrease'&&$delta<0)?'improved':'regressed'));$out[]=$x;}return new WP_REST_Response(['items'=>$out,'count'=>count($out)],200);
 }
