...
export function createQaHandler(source:QaSnapshotSource):JobHandler {
 return async(job,signal)=>{if(!job.siteId)throw new Error('QA_SITE_REQUIRED'); const snapshot=await source.load(job.siteId,signal); if(snapshot.siteId!==job.siteId)throw new Error('QA_SITE_MISMATCH'); const findings=validateSnapshot(snapshot); return {siteId:job.siteId,capturedAt:new Date().toISOString(),findings,summary:summarizeQa(findings)};};
}
