...
import { buildWebsiteSnapshot, summarizeQa, validateSnapshot, type DiscoveredFeature } from './discovery';
...
export interface QaSnapshotSource { load(siteId:string,signal:AbortSignal):Promise<{siteId:string;url:string;capturedAt:string;routes:string[];features:DiscoveredFeature[];technologies:string[]}>; }
