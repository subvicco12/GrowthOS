...
export interface NonceStore {
  consume(siteId: string, nonce: string, expiresAt: Date): Promise<boolean>;
}

export function canonicalConnectorPayload(input: { siteId:string; timestamp:string; nonce:string; requestId:string }, body: string): string {
  return [input.siteId,input.timestamp,input.nonce,input.requestId,body].join('\n');
}
...
export async function authenticateConnector(rawEnvelope: unknown, rawBody: string, secret: string, nonces: NonceStore): Promise<{siteId:string;requestId:string}> {
  ...
  if (!Number.isFinite(timestamp) || Math.abs(Date.now() - timestamp) > MAX_CLOCK_SKEW_MS) throw new Error('STALE_CONNECTOR_REQUEST');
