...
export class PostgresDashboardRepository {
 ...
 async load(siteId?:string):Promise<DashboardQuery>{
  ...
  const mapped=rec.rows.map(r=>({id:r.id,siteId:r.site_id,title:r.title,category:r.category,score:Number(r.score),approvalClass:r.approval_class,status:r.status,evidence:r.evidence??[],impact:r.impact,confidence:r.confidence,effort:r.effort,risk:r.risk}));
