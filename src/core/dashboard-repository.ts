...
export class PostgresDashboardRepository {
 ...
 async load(siteId?:string):Promise<DashboardQuery>{
  ...
  const connectors=await this.db.query<{kind:string;status:'needs_connection'|'connected'|'degraded'|'disabled';last_seen_at:string|null}>(`select kind,status,last_seen_at from public.connectors${siteId?' where site_id=$1':''} order by kind`,params);
  ...
  return {recommendations:mapped,approvals:mapped.filter(x=>x.status==='proposed') as ApprovalInboxItem[],integrations:connectors.rows.map(row=>({name:row.kind,status:row.status==='connected'?'healthy':row.status==='disabled'?'disabled':'attention',detail:row.status==='connected'?(row.last_seen_at?'Connected':'Connected; awaiting first heartbeat'):row.status.replaceAll('_',' ')})),counts,activeJobs:counts.activeJobs};
 }
}
