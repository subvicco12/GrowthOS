...
test('operational dashboard is bounded and defaults safely to empty state', async()=>{ const {buildOperationalDashboard}=await import('../operational-dashboard'); const empty=buildOperationalDashboard(); assert.equal(empty.counts.needsApproval,0); const model=buildOperationalDashboard({counts:{needsApproval:4},nextBestActions:['1','2','3','4','5','6'],recentActivity:Array.from({length:12},(_,i)=>String(i))}); assert.equal(model.counts.needsApproval,4); assert.equal(model.nextBestActions.length,5); assert.equal(model.recentActivity.length,10); });
