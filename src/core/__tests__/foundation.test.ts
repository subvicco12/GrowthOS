...
test('action center prioritizes next best actions and separates approval inbox', async()=>{ const {buildNextBestActions,buildApprovalInbox}=await import('../action-center'); const items:any[]=[{id:'1',siteId:'s',title:'Low',category:'qa',score:2,approvalClass:'green',status:'proposed'},{id:'2',siteId:'s',title:'Pricing',category:'packaging',score:5,approvalClass:'amber',status:'proposed',evidence:['e'],impact:5,confidence:5,effort:2,risk:2},{id:'3',siteId:'s',title:'Verified',category:'qa',score:9,approvalClass:'green',status:'verified'}]; assert.equal(buildNextBestActions(items,2)[0].id,'2'); assert.equal(buildNextBestActions(items,2).length,2); assert.equal(buildApprovalInbox(items).length,1); assert.equal(buildApprovalInbox(items)[0].id,'2'); });
