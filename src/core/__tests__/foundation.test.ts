...
test('action center prioritizes next best actions and separates approval inbox', async()=>{ const {buildNextBestActions,buildApprovalInbox}=await import('../action-center'); const items:any[]=[{id:'1',siteId:'s',title:'Low',category:'qa',score:2,approvalClass:'green',status:'proposed',reason:'Low priority'},{id:'2',siteId:'s',title:'Pricing',category:'packaging',score:5,approvalClass:'amber',status:'proposed',reason:'Pricing gap',evidence:['e'],impact:5,confidence:5,effort:2,risk:2},{id:'3',siteId:'s',title:'Verified',category:'qa',score:9,approvalClass:'green',status:'verified',reason:'Already verified'}]; assert.equal(buildNextBestActions(items,2)[0].id,'2'); assert.equal(buildNextBestActions(items,2).length,2); assert.equal(buildApprovalInbox(items).length,1); assert.equal(buildApprovalInbox(items)[0].id,'2'); });

test('dashboard API validates optional site scope', async()=>{ const {validateDashboardSite,dashboardResponse}=await import('../dashboard-api'); assert.doesNotThrow(()=>validateDashboardSite(undefined)); assert.doesNotThrow(()=>validateDashboardSite('1')); assert.throws(()=>validateDashboardSite('123e4567-e89b-12d3-a456-426614174000'),/INVALID_SITE/); assert.throws(()=>validateDashboardSite('site-1'),/INVALID_SITE/); const snap:any={counts:{},activeJobs:0,nextBestActions:[],approvalInbox:[],integrations:[],generatedAt:'x'}; assert.equal(dashboardResponse(snap).ok,true); });

test('site scope resolves portfolio options without inventing sites', async()=>{ const {resolveSiteScope,siteScopeOptions}=await import('../site-scope'); const sites:any=[{name:'A',domain:'a.example',status:'connected'},{name:'B',domain:'b.example',status:'ready_to_connect'}]; assert.equal(resolveSiteScope(sites,'a.example')?.name,'A'); assert.equal(resolveSiteScope(sites,'missing.example'),undefined); assert.deepEqual(siteScopeOptions(sites).map(x=>x.value),['','a.example','b.example']); });

test('recommendation detail exposes evidence risk and allowed lifecycle actions', async()=>{ const {buildRecommendationDetail}=await import('../recommendation-detail'); const base:any={id:'r',siteId:'s',category:'packaging',title:'Improve API tier',evidence:['pricing page','feature matrix'],impact:5,confidence:4,effort:3,recurringCostUsd:2,risk:5,score:3,approvalClass:'red',status:'proposed',createdAt:'x'}; const d=buildRecommendationDetail(base); assert.equal(d.decisionRequired,true); assert.deepEqual(d.allowedTransitions,['approved','rejected','deferred']); assert.equal(d.executionReady,false); assert.equal(d.riskSummary,'High risk — explicit approval required'); const approved=buildRecommendationDetail({...base,status:'approved'}); assert.equal(approved.executionReady,true); });

test('dashboard repository supports portfolio scope without malformed SQL', async()=>{ const {PostgresDashboardRepository}=await import('../dashboard-repository'); const sql:string[]=[]; const db:any={query:async(q:string)=>{sql.push(q);if(q.includes('from public.recommendations'))return{rows:[]};if(q.includes('from public.connectors'))return{rows:[]};return{rows:[{count:'0'}]};}}; await new PostgresDashboardRepository(db).load(); assert.ok(sql[0].includes('where status not in')); assert.ok(!sql[0].includes('undefined')); });

test('approval commands normalize inputs and map UI actions to lifecycle states', async()=>{ const {normalizeApprovalCommand,targetStatusForApproval}=await import('../approval-command'); const x=normalizeApprovalCommand({recommendationId:' r ',actorId:' u ',action:'approve',note:' ok ',idempotencyKey:' k '}); assert.equal(x.recommendationId,'r'); assert.equal(x.note,'ok'); assert.equal(targetStatusForApproval(x.action),'approved'); assert.throws(()=>normalizeApprovalCommand({recommendationId:'',actorId:'u',action:'approve',idempotencyKey:'k'}),/RECOMMENDATION_ID_REQUIRED/); assert.throws(()=>normalizeApprovalCommand({recommendationId:'r',actorId:'u',action:'approve',idempotencyKey:'k',note:'x'.repeat(2001)}),/APPROVAL_NOTE_TOO_LONG/); });

test('approval inbox view model exposes only governed actions', async()=>{ const {buildApprovalInboxViewModel}=await import('../approval-inbox-view'); const out=buildApprovalInboxViewModel([{id:'r',title:'Fix',siteId:'s',category:'qa',score:4,approvalClass:'red',risk:5,impact:5,confidence:4,effort:2,evidence:['url'] }]); assert.deepEqual(out[0].actions,['approve','reject','defer']); });

test('approval dispatch normalizes UI action payload before transport', async()=>{ const {buildApprovalDispatch}=await import('../approval-dispatch'); const out=buildApprovalDispatch({recommendationId:' r ',action:'approve',actorId:' u ',note:' note ',idempotencyKey:' k '}); assert.deepEqual(out,{recommendationId:'r',action:'approve',actorId:'u',note:'note',idempotencyKey:'k'}); });

test('dashboard service validates scope before loading persistence', async()=>{ const {DashboardService}=await import('../dashboard-service'); let loaded=false; const repo:any={load:async()=>{loaded=true;return {recommendations:[],approvals:[],integrations:[],counts:{needsApproval:0,inDevelopment:0,inProduction:0,qaFailed:0,readyForReview:0,readyForListing:0,readyToPublish:0,live:0,exceptions:0,activeJobs:0},activeJobs:0}}}; const service=new DashboardService(repo); await assert.rejects(()=>service.get('bad'),/INVALID_SITE/); assert.equal(loaded,false); const out=await service.get(); assert.equal(out.activeJobs,0); });

test('dashboard route maps validation failures to 400 without exposing persistence errors', async()=>{ const {DashboardService,createDashboardRoute}=await import('../dashboard-service'); const repo:any={load:async()=>({recommendations:[],approvals:[],integrations:[],counts:{needsApproval:0,inDevelopment:0,inProduction:0,qaFailed:0,readyForReview:0,readyForListing:0,readyToPublish:0,live:0,exceptions:0,activeJobs:0},activeJobs:0})}; const route=createDashboardRoute(new DashboardService(repo)); const bad=await route.get('bad'); assert.equal(bad.status,400); assert.equal(bad.body.code,'INVALID_SITE'); });

test('command center adapter exposes live counts and governed approval items', async()=>{ const {buildCommandCenterViewModel}=await import('../command-center-view'); const snapshot:any={counts:{needsApproval:1,exceptions:2},activeJobs:3,nextBestActions:[{id:'n'}],approvalInbox:[{id:'r',title:'Review',siteId:'s',category:'packaging',score:4,approvalClass:'amber',risk:2,impact:4,confidence:4,effort:2,evidence:['pricing']}],integrations:[],generatedAt:'x'}; const vm=buildCommandCenterViewModel(snapshot,6); assert.deepEqual(vm.metrics,{websites:6,activeJobs:3,needsApproval:1,exceptions:2}); assert.deepEqual(vm.approvalInbox[0].actions,['approve','reject','defer']); });

test('dashboard repository maps connector control-plane status without inventing health columns', async()=>{ const {PostgresDashboardRepository}=await import('../dashboard-repository'); let call=0; const db:any={query:async(q:string)=>{call++; if(q.includes('recommendations'))return{rows:[]}; if(q.includes('jobs'))return{rows:[{count:'0'}]}; if(q.includes('connectors'))return{rows:[{kind:'github',status:'connected',last_seen_at:'2026-09-22T00:00:00Z'},{kind:'search_console',status:'needs_connection',last_seen_at:null}]}; return{rows:[]};}}; const out=await new PostgresDashboardRepository(db).load(); assert.equal(out.integrations[0].status,'healthy'); assert.equal(out.integrations[1].status,'attention'); assert.equal(call,3); });

test('integration health classifies deterministic connector telemetry', async()=>{ const {integrationHealth}=await import('../integration-health'); const now=new Date('2026-09-22T12:00:00Z'); const out=integrationHealth([{name:'fresh',enabled:true,last_success_at:'2026-09-22T11:00:00Z',last_error:null},{name:'stale',enabled:true,last_success_at:'2026-09-20T00:00:00Z',last_error:null},{name:'off',enabled:false,last_success_at:null,last_error:null},{name:'error',enabled:true,last_success_at:null,last_error:'timeout'}],now); assert.deepEqual(out.map(x=>x.status),['healthy','attention','disabled','attention']); });

test('approval service returns exact idempotent retry without duplicate mutation', async()=>{ const {PostgresRecommendationApprovalService}=await import('../recommendation-approval-service'); const row:any={id:'r1',site_id:'s1',category:'seo',title:'Fix',evidence:[],impact:3,confidence:3,effort:1,recurring_cost_usd:0,risk:1,score:3,approval_class:'amber',status:'approved',created_at:'x'}; const sql:string[]=[]; const db:any={query:async(q:string)=>{sql.push(q);if(q.includes('from public.approvals'))return{rows:[{recommendation_id:'r1',actor_id:'a1',decision:'approved'}]};if(q.includes('from public.recommendations'))return{rows:[row]};return{rows:[]};}}; const out=await new PostgresRecommendationApprovalService(db).decide({recommendationId:'r1',actorId:'a1',decision:'approved',idempotencyKey:'k1'}); assert.equal(out.status,'approved'); assert.equal(sql.some(q=>q.startsWith('update public.recommendations')),false); assert.equal(sql.some(q=>q.includes('insert into public.audit_events')),false); });

test('approval service rejects conflicting idempotency-key reuse', async()=>{ const {PostgresRecommendationApprovalService}=await import('../recommendation-approval-service'); const db:any={query:async(q:string)=>q.includes('from public.approvals')?{rows:[{recommendation_id:'other',actor_id:'a1',decision:'approved'}]}:{rows:[]}}; await assert.rejects(()=>new PostgresRecommendationApprovalService(db).decide({recommendationId:'r1',actorId:'a1',decision:'approved',idempotencyKey:'k1'}),/APPROVAL_IDEMPOTENCY_CONFLICT/); });

test('approval command adapter maps UI action to transactional decision and detail', async()=>{ const {PostgresApprovalCommandAdapter}=await import('../approval-command-adapter'); let decision=''; const svc:any={decide:async(input:any)=>{decision=input.decision;return{id:'r',siteId:'s',category:'seo',title:'Fix',evidence:[],impact:3,confidence:4,effort:1,recurringCostUsd:0,risk:2,score:3,approvalClass:'amber',status:'approved',createdAt:'x'}}}; const out=await new PostgresApprovalCommandAdapter(svc).decide({recommendationId:'r',actorId:'u',action:'approve',idempotencyKey:'k'}); assert.equal(decision,'approved'); assert.equal(out.executionReady,true); });

test('approval transport rejects malformed body and returns governed result', async()=>{ const {handleApprovalRequest}=await import('../approval-transport'); const svc:any={decide:async()=>({id:'r',siteId:'s',category:'seo',title:'Fix',evidence:[],impact:3,confidence:4,effort:1,recurringCostUsd:0,risk:2,score:3,approvalClass:'amber',status:'approved',createdAt:'x',decisionRequired:false,allowedTransitions:[],evidenceSummary:[],riskSummary:'Low risk',executionReady:true})}; assert.equal((await handleApprovalRequest(svc,null)).status,400); const ok=await handleApprovalRequest(svc,{recommendationId:'r',actorId:'u',action:'approve',idempotencyKey:'k'}); assert.equal(ok.status,200); });

test('approval route requires authenticated privileged actor and ignores client actor id', async()=>{ const {createApprovalRoute}=await import('../approval-route'); const denied=createApprovalRoute({service:{} as any,authenticate:async()=>null}); assert.equal((await denied(new Request('http://x',{method:'POST',body:'{}'}))).status,401); const forbidden=createApprovalRoute({service:{} as any,authenticate:async()=>({id:'u',role:'viewer'})}); assert.equal((await forbidden(new Request('http://x',{method:'POST',body:'{}'}))).status,403); let actor=''; const service:any={decide:async(input:any)=>{actor=input.actorId;return {id:'r',siteId:'s',category:'seo',title:'Fix',evidence:[],impact:3,confidence:4,effort:1,recurringCostUsd:0,risk:2,score:3,approvalClass:'amber',status:'approved',createdAt:'x',decisionRequired:false,allowedTransitions:[],evidenceSummary:[],riskSummary:'Low risk',executionReady:true};}}; const route=createApprovalRoute({service,authenticate:async()=>({id:'server-user',role:'owner'})}); const res=await route(new Request('http://x',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({recommendationId:'r',action:'approve',idempotencyKey:'k'})})); assert.equal(res.status,200); assert.equal(actor,'server-user'); });
