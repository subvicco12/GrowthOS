...
export default function Dashboard() {
 ...
 return <main className="shell">
  ...
  <section className="content">
   ...
   <div className="metrics"><article><span>Websites</span><strong>{snapshot?snapshot.sites.length:'—'}</strong><small>central registry</small></article><article><span>Connected</span><strong>{snapshot?snapshot.integrations.filter(x=>x.status==='healthy').length:'—'}</strong><small>live connector status</small></article><article><span>Active jobs</span><strong>{counts?.activeJobs??'—'}</strong><small>queued + running</small></article><article><span>Needs approval</span><strong>{counts?.needsApproval??'—'}</strong><small>AMBER / RED decisions</small></article></div>
   <section className="panel"><div className="panelHead"><div><h2>Operations at a glance</h2><p>Live operational signals from the GrowthOS control plane.</p></div><span className="engineCount">Control plane</span></div><div className="workspaceGrid">{operationalItems.map(([label,key])=><article className="workspace" key={key}><strong>{label}</strong><span>{counts?.[key as keyof typeof counts]??'—'}</span></article>)}</div></section>
   <ApprovalInboxPanel items={approvals}/>
   <section className="panel" id="portfolio"><div className="panelHead"><div><h2>Website Portfolio</h2><p>{siteId&&selectedSite?'Scoped to '+selectedSite.name+'.':'All six current properties are registered in the command center.'}</p></div><button type="button" className="secondary">Run portfolio audit</button></div><div className="siteGrid">{(snapshot?snapshot.sites:[]).map(site=><article className="site" key={site.domain}><div className="siteIcon">{site.name.slice(0,2).toUpperCase()}</div><div><h3>{site.name}</h3><p>{site.domain}</p><span className="status">{site.status}</span></div><button type="button" className="ghost">Open</button></article>)}</div>{!loading&&!error&&snapshot?.sites.length===0&&<p className="emptyState">No live site registry rows returned for this scope.</p>}</section>
