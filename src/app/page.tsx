...
export default function Dashboard() {
 ...
 return <main className="shell">
  ...
  <section className="content">
   ...
   <div className="metrics"><article><span>Websites</span><strong>{snapshot?.sites.length??(siteId?(selectedSite?1:0):sites.length)}</strong><small>central registry</small></article><article><span>Connected</span><strong>{snapshot?.integrations.filter(x=>x.status==='healthy').length??0}</strong><small>live connector status</small></article><article><span>Active jobs</span><strong>{counts?.activeJobs??0}</strong><small>queued + running</small></article><article><span>Needs approval</span><strong>{counts?.needsApproval??0}</strong><small>AMBER / RED decisions</small></article></div>
